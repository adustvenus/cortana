...
| No mic / mic unplugged | nothing to capture with | She stays up now (this used to crash the process): F9 answers "no input device", F10 refuses to leave PTT, and plugging a mic in needs no restart. Pick the device from the AI module's **MIC** picker on the dashboard, or set `MIC_NAME=`/`MIC_DEVICE=` in `.env` |
| Wrong mic picked (e.g. HDMI) | ALSA default device confusion | AI module **MIC** picker, or `python -c "import sounddevice as sd; print(sd.query_devices())"` → set `MIC_DEVICE=` index in `.env` |
